...
document.addEventListener('DOMContentLoaded', function () {
    ...
    botoesVerReservas.forEach(botao => {
        ...
        const idtipo_quarto = this.getAttribute('reservas');
...
// Script para abrir o modal de edição de tipo de quarto
document.querySelectorAll(".btn-editar").forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      fetch("tquartos.php", {
        ...
        body: new URLSearchParams({ action: "buscar_tipoQuarto", id: id }),
      })
        .then((res) => res.json())
        .then((data) => {
          if (data.error) {
            ...
            document.getElementById('edittipoQuarto').value = data.tipoQuarto;
        });
    });
  });

// Script para atualizar os dados do hóspede no modal de edição
document.getElementById("formEditarTipoQuarto").addEventListener("submit", function (e) {
    e.preventDefault();
    ...
    fetch("tquartos.php", {
      ...
      .then((response) => {
        ...
        if (response.success) {
          showNotificationModal("Ocupação atualizada com sucesso!", 'success');
        } else {
...
document.querySelectorAll(".btn-remover").forEach(btn => {
    btn.addEventListener("click", () => {
        ...
        if (info.error) {
            ...
            msgEl.innerHTML = `
            <p>Não é possível remover este tipo de quarto.</p>
            <p>Há <strong>${info.count}</strong> registro(s) na tabela <strong>${info.table}</strong> que depende(m) deste tipo de quarto.</p>
            `;
            footerEl.innerHTML = '<button class="btn btn-secondary" data-bs-dismiss="modal"> Entendido </button>';
        } else {
            // pode remover — confirmação
            msgEl.textContent = "Tem certeza que deseja remover este tipo de quarto?";
            footerEl.innerHTML = `
            ...
            .addEventListener("click", () => {
                ...
                method: "POST",
                headers: { "Content-Type": "application/x-www-form-urlencoded" },
                body: new URLSearchParams({ action: "remover_tquarto", id })
                })
                .then(res => res.json())
                .then(resp => {
