...
document.addEventListener('DOMContentLoaded', function () {
    ...
    botoesVerReservas.forEach(botao => {
        ...
        const idocupacao = this.getAttribute('reservas');
        ...
        fetch('ocupacoes.php', {
            ...
            body: JSON.stringify({ idocupacao: idocupacao })
        })
...
document.querySelectorAll(".btn-editar").forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      fetch("ocupacoes.php", {
        ...
        body: new URLSearchParams({ action: "buscar_ocupacao", id: id }),
      })
        .then((res) => res.json())
        .then((data) => {
          if (data.error) {
          ...
          document.getElementById("editOcupacao").value = data.ocupacao;
        });
    });
  });

// Script para atualizar os dados do hóspede no modal de edição
document.getElementById("formEditarOcupacao").addEventListener("submit", function (e) {
    e.preventDefault();
...
btn.addEventListener("click", () => {
    ...
    if (info.error) {
        ...
        msgEl.innerHTML = `
        <p>Não é possível remover esta ocupação.</p>
        <p>Há <strong>${info.count}</strong> registro(s) na tabela <strong>${info.table}</strong> que depende(m) desta ocupação.</p>
        `;
        footerEl.innerHTML = '<button class="btn btn-secondary" data-bs-dismiss="modal"> Entendido </button>';
    } else {
        // pode remover — confirmação
        msgEl.textContent = "Tem certeza que deseja remover esta ocupação?";
        footerEl.innerHTML = `
        ...
        .addEventListener("click", () => {
            ...
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
            body: new URLSearchParams({ action: "remover_ocupacao", id })
            })
            .then(res => res.json())
            .then(resp => {
